...
public class SharedInventoryGame {
    ...
    // Name of a player that changed the inventory (set to "" when command was
    // initiated)
    ...
    public void addPlayers(CommandSender p, String[] args) {
        ...
        if (args[2].equals("@a")) {
            ...
            playersToAdd = playersToAdd.stream().filter(name -> Bukkit.getPlayerExact(name) != null)
        ...
        if (count > 0) {
            p.sendMessage(ChatColor.AQUA + "Successfully added " + count + " player" + (count == 1 ? "" : "s")
                    + " to the game!");
        } else {
    ...
    public void removePlayers(CommandSender p, String[] args) {
        ...
        if (count > 0) {
            p.sendMessage(ChatColor.AQUA + "Successfully removed " + count + " player" + (count == 1 ? "" : "s")
                    + " from the game!");
        } else {
                ...
                for (SharedInventoryPlayer playerObject : players) {
                    ...
                    if (!inventoryEqual(sharedInventory, player.getInventory()) && !isPlayerBreakingBlock(player)) {
                        ...
                        if (playerWithDifferentInventory == null)
                            playerWithDifferentInventory = player;
                    }
                }

                if (changedInventories >= 1) {
                    if ((changedInventories > 1 || playerNameWithDifferentInventory == null) && !commandInitiated
                            && main.getConfig().getBoolean("showSyncErrors")) {
                        playersMessage(ChatColor.DARK_RED
                                + "Sync error! Someone has changed the inventory in a not implemented way!");
                    }
        ...
        if (inGame) {
            ...
            if (player != null) {
                ...
                if (main.getConfig().getBoolean("clearInventories") || !name.equals(firstPlayer)) {
                    if ((playerObject.getOldInventory() != null))
